...
<script>
  canvas.onpaint = () => {
    ...
    ctx.reset();
    // Note: Without specifying `preserveElementGeometry: false`, the following
    // `drawElementImage` calls automatically update geometry, ensuring the
    // image and caption have geometry information for accessibility.
